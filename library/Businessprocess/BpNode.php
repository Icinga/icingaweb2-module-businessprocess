    ...
    public function reCalculateState()
    {
        ...
        switch ($this->getOperator()) {
            ...
            case self::OP_DEGRADED:
                ...
                $maxIcingaState = $maxState >> self::SHIFT_FLAGS;
                $flags = $maxState & 0xf;

                $warningState = ($this->stateToSortStateMap[self::ICINGA_WARNING] << self::SHIFT_FLAGS) + $flags;

                $sort_state = ($maxIcingaState === $this->stateToSortStateMap[self::ICINGA_CRITICAL]) ?
                    $warningState : $maxState;
                break;
            default:
                // MIN:
